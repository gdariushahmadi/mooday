    ...
    <div className="w-full flex flex-col gap-lg pb-10">
      ...
      <nav className="flex items-center justify-between border-b border-surface-variant">
        <div className="flex space-x-gutter overflow-x-auto no-scrollbar pb-3">
          ...
          <button className="pb-1 border-b-2 border-transparent text-on-surface-variant hover:text-on-surface transition-colors uppercase tracking-widest text-label-md whitespace-nowrap px-1">
            {isAr ? "الرائج" : "Trending"}
          </button>
          <button className="pb-1 border-b-2 border-transparent text-on-surface-variant hover:text-on-surface transition-colors uppercase tracking-widest text-label-md whitespace-nowrap px-1">
            {isAr ? "المصممون" : "Designers"}
          </button>
        </div>
      ...
      >
        <div className="flex items-center justify-between gap-md">
          ...
          >
            ...
            >
              <span className="material-symbols-outlined text-[20px] no-mirror">view_agenda</span>
            </button>
            <button
            ...
            >
              <span className="material-symbols-outlined text-[20px] no-mirror">grid_view</span>
            </button>
          </div>
        </div>
      ...
      {filteredListings.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 gap-md text-center">
          <span className="material-symbols-outlined text-[56px] text-outline opacity-40">category</span>
          <p className="text-body-lg text-on-surface-variant">
            {isAr ? "لا توجد منتجات في هذه الفئة." : "No items in this category yet."}
          </p>
        </div>
      ) : viewMode === "compact" ? (
        <section
            ...
            return (
              ...
              >
                ...
                <div className="p-md flex flex-col gap-1">
                  ...
                  <span className="font-bold text-primary text-label-sm">AED {product.price}</span>
                </div>
              </article>
            );
          })}
      ...
      <section aria-label="Discover Feed" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-lg">
        {filteredListings.map((product, idx) => {
          const isFeatured = idx === 0; // Make first item featured and occupy 2 columns on medium screens
          const isLiked = likes.includes(product.id);
            ...
            >
              ...
              <div className="absolute top-md left-md z-10 bg-white/70 backdrop-blur-md px-3 py-1 rounded-full border border-white/50">
                <span className="font-bold uppercase tracking-widest text-label-sm text-primary flex items-center gap-1">
                  ...
                  {isFeatured 
                    ? (isAr ? "الرائج" : "Trending") 
                    : (product.isAuthentic ? (isAr ? "موثوق" : "Authentic") : (isAr ? "ممتاز" : "Excellent"))
                  }
                </span>
              </div>
              ...
              {/* Product Image */}
              <div className={`${isFeatured ? "aspect-[16/9]" : "aspect-[4/5]"} w-full bg-surface-container-low overflow-hidden`}>
                <img
              ...
              <div className="p-lg flex flex-col gap-md">
                <div className="flex justify-between items-start gap-4">
                  <div className="flex-1">
                    ...
                    <h2 className={`${isFeatured ? "text-headline-sm" : "text-[18px]"} font-serif text-on-surface group-hover:text-primary transition-colors`}>
                      {isAr ? product.titleAr : product.titleEn}
                    </h2>
                    {isFeatured && (
                      <p
                  ...
                  <div className="text-right flex-shrink-0">
                    <span className="block text-on-surface-variant line-through text-body-md opacity-60">
                      {isAr ? `AED ${product.originalPrice}` : `AED ${product.originalPrice}`}
                    </span>
                    <span className={`block font-bold text-primary ${isFeatured ? "text-headline-sm" : "text-label-md"}`}>
                      {isAr ? `AED ${product.price}` : `AED ${product.price}`}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between pt-md border-t border-surface-container-high mt-2">
                  ...
                  <div className="flex items-center gap-1 text-on-surface-variant opacity-70">
                    ...
                    <span className="text-label-sm">{product.saves + (isLiked ? 1 : 0)}</span>
                  </div>
                </div>
              </div>
            </article>
